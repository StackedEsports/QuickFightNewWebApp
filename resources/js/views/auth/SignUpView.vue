...
async function registerUser(userData) {
  try {
    ...
    if (error.response && error.response.data.error === 'Email already in use') {
      state.emailError = "This email is already in use. Please use a different email.";
    } else {
...
async function onSubmit() {
  ...
  const userData = {
    ...
    password_confirmation: state.confirmPassword, // Added this line
    // Include other necessary data as per your backend requirements
  };
                ...
                <div class="py-3">
                  ...
                  <div class="mb-4">
                    <input
                      ...
                      name="signup-email"
                      placeholder="Email"
                      :class="{'is-invalid': v$.email.$errors.length || state.emailError}"
                      v-model="state.email"
                      @blur="v$.email.$touch"
                    />
                    <div v-if="state.emailError" class="invalid-feedback animated fadeIn">
                      {{ state.emailError }}
                    </div>
                    <div v-else-if="v$.email.$errors.length" class="invalid-feedback animated fadeIn">
